...
for split in "${splits[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        task_name=relu_${model}_pretrained_${retain_split} \
        model=${model}
...
for split in "${splits[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        eval.relu.retain_split=${retain_split} \
        eval.relu.holdout_split=${holdout_split} \
        task_name=relu_${model}_full_${retain_split} \
        model=${model} \
...
for split in "${splits[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        eval.relu.retain_split=${retain_split} \
        eval.relu.holdout_split=${holdout_split} \
        task_name=relu_${model}_${retain_split} \
        model=${model} \
...
#     for model in "${models[@]}"; do
#         CUDA_VISIBLE_DEVICES=1,1 accelerate launch --config_file configs/accelerate/default_config.yaml --main_process_port $MASTER_PORT \
